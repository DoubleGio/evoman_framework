...
def parent_selection(pop, fit_pop, gen_i, n_parents=3):
    parents = np.zeros((n_parents, pop.shape[1]))
    percentage = (10 + (gen_i / N_GENS) * 40) / 100  # Controlling tournament size deterministically
...
def crossover(pop, fit_pop, n_vars, gen_i):
    ...
    for p in range(0, pop.shape[0], 2):  # Loop for npop / 2
        p1, p2, p3 = parent_selection(pop, fit_pop, gen_i)
        n_offspring = 3
        ...
        for f in range(0, n_offspring):
            ...
            for i in range(0, len(offspring[f])):
                if np.random.uniform(0, 1) <= MUTATION:
                    s = 1 - 0.9 * (gen_i / N_GENS)  # Controlling noise deterministically
...
def plot(n_tests, enemies):
    ...
    for i in range(n_tests):
        file = np.loadtxt(f'EAGio results/enemy{enemies}_test{i + 1}/results.txt', skiprows=1, max_rows=N_GENS)
